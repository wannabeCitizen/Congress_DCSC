...
def parse_directory(path, my_date, **kwargs):
    logfile = initialize_logfile(kwargs['logdir'])
    
    # This will all need changed once I know the node numbers
    ...
    for file in os.listdir(path):
        ...
        # Makes text versions for the parser
            ...
            ###
            ## Here is the content of the text raw text document
            ###
        ...
        xml_path = do_parse(parser, logfile)

        try:
            xml_parse_helper = lxml.etree.XMLParser(recover=True)
            print "\n\n XML PATH: {0} \n\n".format(xml_path)

            xml_tree = lxml.etree.parse(xml_path, parser=xml_parse_helper)
            xml_root = xml_tree.getroot()
            speaker_list = xml_root.findall('.//*[@name]')
            unique_speak = {}
